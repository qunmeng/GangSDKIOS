...
@implementation GangChatResponseRightTableViewCell
...
- (void)awakeFromNib {
    ...
    [self.btn_response setTitleColor:[UIColor colorFromHexRGB:GangColor_gangChat_responseButtonTitle] forState:UIControlStateNormal];
...
-(float)computeTheCellHeight:(GangChatMessageBean*)obj{
    self.label_content.attributedText = [CodoneTools getshowContent:obj.message textColor:[UIColor colorFromHexRGB:GangColor_gangChat_messageColor_right] font:[UIFont systemFontOfSize:11] lineSpace:0 paraSpace:0];
...
-(void)setTheObj:(GangChatMessageBean*)obj{
    ...
    self.label_content.attributedText = [CodoneTools getshowContent:obj.message textColor:[UIColor colorFromHexRGB:GangColor_gangChat_messageColor_right] font:[UIFont systemFontOfSize:11] lineSpace:4 paraSpace:0];
    CGSize size = [self.label_content sizeThatFits:CGSizeMake([UIScreen mainScreen].bounds.size.width-(23+50+5+20+8+46+9+60)+width_flag, 0)];
    self.constraint_width_label_content.constant = size.width;
    self.constraint_height_label_content.constant = size.height;
    size.width += 20+8+46+9;
...
- (IBAction)btn_response_click:(UIButton *)sender {
}
