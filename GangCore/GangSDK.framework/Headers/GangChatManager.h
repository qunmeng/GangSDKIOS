...
#define Gang_notify_receiveSingleMessage        @"GangReceiveSingleMessaged"         /**<收到私聊频道消息,这条消息会被放入messages_world中，同时会携带在notification.userInfo<@"data":GangChatMessageBean>*/
...
#define Gang_notify_clickedASpecialCell        @"GangClickedASpecialCell"            /**<点击了一条特殊消息,消息会携带在notification.userInfo<@"data":GangMessageSpecialBean,@"navigationController":UINavigationController>*/
#define Gang_notify_receiveGangNotifyMessage   @"GangReceiveGangNotifyMessage"      /**<收到通知消息*/
...
@property(strong) NSMutableArray<GangChatMessageBean*> * _Nullable messages_single;         /**<私聊频道消息缓存*/
...
/**
 *  发送特殊消息
 *  @param message  特殊格式的内容
 *  @param channel  频道(1-招募频道，2-社群频道)
 */

- (void)sendSpecialMessage:(GangMessageSpecialBean *_Nonnull)message
                 inChannel:(int)channel
                   success:(void (^_Nullable)(void))success
                      fail:(void (^_Nullable)(NSError * _Nullable error))failure;
...
- (void)sendSingleChatTextMessage:(NSString *_Nonnull)message
                         toUserId:(NSString *_Nonnull)targetuserid
                          success:(void (^_Nullable)(void))success
                             fail:(void (^_Nullable)(NSError * _Nullable error))failure;
 ...
 *  @param  pageSize  每页条数
 *  @param channel   频道(1-招募频道，2-社群频道，3-私聊频道)
 */
