...
+ (NSString *)jumpToBizPay:(NSString *)userid
    ...
    NSDictionary *params = @{@"userid":userid,@"productid":productid};
    
    NSString *urlString = [NSString stringWithFormat:@"%@?%@", @"", [NSString stringWithFormat:@"json=%@",[params objectForKey:@"json"]]];
    //解析服务端返回json数据
    NSError *error;
    //加载一个NSURL对象
    ...
    if ( response != nil) {
        ...
        NSLog(@"url:%@",urlString);
        if(dict != nil){
            NSMutableString *result_code = [dict objectForKey:@"result_code"];
